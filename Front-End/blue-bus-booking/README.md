# 🖥️ BlueBus — React Front-End Client Application
...
The modern, highly responsive React single-page client application for the BlueBus Platform. Built using React 19, Vite 8, and the advanced Tailwind CSS v4 styling engine, the frontend provides intuitive user booking flows, interactive mapping, an AI Chatbot widget, and exhaustive dashboards for both Operators and System Administrators.
...
> [!NOTE]
> Screenshot assets are located in the repository's root `../docs/screenshots/` directory.
...
## ✨ Code Modules & Architecture

The client-side React code is divided into key modules located inside `src/Components/`:

- **Page1 (Home)**: High-resolution search deck capturing departure/arrival stations and scheduling calendar.
- **Page2 (Search Results & Seat Selector)**: 
  - Dynamic route lists containing travel intervals, prices, and seats remaining.
  - Comprehensive seat allocation layout for single/double-decker buses.
  - **AI Recommendation Engine Alert**: Prompts passengers with preferred seating (e.g., suggesting a window or aisle seat on the upper deck based on historical journeys).
- **Payments**: Drawer interface checking out passenger age/gender lists, processing coupons, and presenting Razorpay integration.
- **Bookings (Confirmed Boarding)**: Renders confirmation statistics, barcode verification scans, and triggers PDF downloads.
- **AI Chat**: Floating chat drawer allowing passengers to query BlueBus Concierge for instant schedule, booking status, and cancellation assistance.
- **Leaflet Maps Integration**: Renders maps tracing stations and path timelines dynamically.
- **UserComponent**: Handles registration, login (JWT token capture), profile administration (URL avatar management), security password adjustments, and past reservations history.
- **AdminDashBoard**: Centralized metrics summaries, user lists, partner databases, schedules creation, and maintenance purges.
- **OperatorPortal**: Fleet details, schedules configurations, and financial performance reports.
