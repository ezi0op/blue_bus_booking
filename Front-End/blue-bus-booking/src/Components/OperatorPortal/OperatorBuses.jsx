...
const OperatorBuses = () => {
  ...
  const handleToggleStatus = async (busId) => {
    try {
      const response = await api.put(`/api/operator/bus/${busId}/status?operatorId=${operatorId}`);
      if (response.data.success) {
        fetchBuses();
      }
    } catch (error) {
  ...
  const handleAddBus = async (e) => {
    ...
    try {
      if (editingBusId) {
        const response = await api.put(`/api/operator/bus/${editingBusId}?operatorId=${operatorId}`, newBus);
        if (response.data.success) {
          ...
          setTimeout(() => {
            ...
            setNewBus({ busNumber: '', busType: 'AC', totalSeats: 36, image: '' });
      ...
      setMessage({ type: 'error', text: editingBusId ? 'Failed to update bus.' : 'Failed to add bus. Check number format.' });
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[32px] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
             <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
               <div>
                 <h3 className="text-xl font-black text-slate-800 tracking-tight uppercase italic">{editingBusId ? 'Update' : 'Add'} <span className="text-blue-600">Vehicle</span></h3>
                 <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">{editingBusId ? 'Edit registered bus details' : 'Register a new bus to your company'}</p>
               </div>
               <button onClick={() => { setShowModal(false); setEditingBusId(null); }} className="p-2 hover:bg-white rounded-xl text-slate-400 hover:text-slate-600 shadow-sm transition-all">
                 <X size={20} />
               </button>
             </div>
              ...
              <form onSubmit={handleAddBus} className="p-8 space-y-5">
                ...
                >
                  {submitting ? (editingBusId ? 'Updating...' : 'Registering Bus...') : (editingBusId ? 'Update Vehicle' : 'Add to Fleet')}
                </button>
              </form>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-50">
              ...
              <div className="flex gap-2">
                ...
                <button 
                  ...
                  className={`p-2 rounded-xl transition-all ${bus.isActive ? 'hover:bg-red-50 text-red-600' : 'hover:bg-emerald-50 text-emerald-600'}`}
                >
                  <Trash2 size={16} />
                </button>
              </div>
