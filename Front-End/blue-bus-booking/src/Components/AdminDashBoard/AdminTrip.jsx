            ...
            trips.map(trip => (
              <div key={trip.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group overflow-hidden relative">
                ...
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-14 h-14 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center shadow-inner overflow-hidden border border-blue-100">
                    {trip.busImage ? (
                      <img src={trip.busImage} alt="Bus" className="w-full h-full object-cover" />
                    ) : (
                      <BusIcon size={28} />
                    )}
                  </div>
                ...
                <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                  <div className="flex flex-col gap-2">
                    ...
                    <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest text-center ${trip.status === 'SCHEDULED' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'
                      }`}>
                      {trip.status}
                    </span>
                  </div>
                    ...
                    <button onClick={() => {
                      ...
                      if (trip.arrivalTime) {
                        ...
                        if (arrivalTimeStr.includes('T')) {
                          const [date, time] = arrivalTimeStr.split('T');
                          tripCopy.arrivalDate = date;
                          tripCopy.arrivalTimeOnly = time ? time.slice(0, 5) : '';
                        } else {
                          tripCopy.arrivalDate = trip.journeyDate;
                          tripCopy.arrivalTimeOnly = arrivalTimeStr.slice(0, 5);
                        }
                      }
                      setEditingItem(trip);
